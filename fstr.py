...
def notation_abs(x: float, precision: int, round_static: bool=False, trailing_zeros: bool=True) -> str: #converts to formatted rounded number (notation absolute) as string, no changing of magnitude for decimal prefixes
    types.check(notation_abs, locals(), types.Mode.convertable, types.Mode.instance, types.Mode.instance, types.Mode.instance)
    x=float(x)
    ...
    x=f"{x:,.{dec_places}f}".replace(".", "%TEMP%").replace(",", ".").replace("%TEMP%", ",")   #int to str, comma as decimal separator
